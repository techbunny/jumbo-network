...
data "azurerm_key_vault" "existing" {
  ...
  resource_group_name = "${var.base_name}-core-rg"
}


## Collects Data from Pre-Existing IoTHub DPS

data "azurerm_iothub_dps" "existing" {
  name                = "${var.base_name}-iothub-dps"
  resource_group_name = "${var.base_name}-app-rg"
}

data "azurerm_iothub_dps_shared_access_policy" "existing" {
  name                = "provisioningserviceowner"
  resource_group_name = "${var.base_name}-app-rg"
  iothub_dps_name     = data.azurerm_iothub_dps.existing.name
}
...
module "peering" {
  ...
  resource_group_nameA = module.resourcegroup.rg_name
  resource_group_nameB = module.resourcegroup.rg_name
...
## Adds public key to KV (for SSH for VMs)
...
module "create_linuxserver_on_vnet" {
  ...
  ssh_public_key                = azurerm_key_vault_secret.pubkey.value
  dps_key                       = data.azurerm_iothub_dps_shared_access_policy.existing.primary_key
  alt_admin                     = var.alt_admin
  sastoken                      = var.sastoken
